...
// Meal-plan placement is household-specific, so it lives in _scripts/config/mealplan.json
// (git-ignored), with _scripts/config/mealplan.example.json as the shared template and a
...
async function loadMealPlanConfig(params) {
  ...
  try {
    const { app } = env(params);
    for (const path of ["_scripts/config/mealplan.json", "_scripts/config/mealplan.example.json"]) {
