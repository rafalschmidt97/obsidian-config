...
Shared implementation lives in `_scripts/shared/runtime.md`, loaded by QuickAdd and Templater through the vault API. It owns rendering, calendar calculations, org settings, entity discovery, file helpers, route-field rendering, and draft matching. QuickAdd keeps synchronous exports and loads dependencies inside async entry calls; it does not use Node imports or top-level await. The runtime loads local org config, then the example if missing/invalid, then alphabetical ordering with personal last. `_scripts/README.md` documents the loader constraints and test command. The standalone cleanup script has no shared routing dependencies.
...
| `_scripts/quickadd/actionpoints.md` | QuickAdd `Tasks` choice + startup refresh | Generates Tasks, Priority, and Wishlist from open `## Tasks` checkboxes in weekly notes, journals, project profiles, and books. Uses `shared/runtime.md` for common helpers. Manual execution chooses org and view; startup waits for metadata readiness and refreshes all orgs. Source notes remain canonical; snapshots do not write back. |
...
QuickAdd's regular Journal command keeps its org/mode/context choices, with the org-wide `journal` route defaulting to `event` and proceeding straight to `title?`. Sport targets `personal/journal/` and asks only `title?`. Reflection asks `org?` and `title?` for an ad-hoc thinking journal; Monthly Reflection remains the separate period-based flow. Sport and Reflection are immediate entries: no mode prompt, draft creation, or draft activation. Drafts are for events and contextual occurrences such as meetings and 1-1s. Folder-click draft mode offers only `event` on the org-wide route. `none` remains valid on older notes but is no longer a QuickAdd capture choice.

New sport journals use `_templates/Journal Sport.md` in both QuickAdd and folder-click capture. It embeds `_templates/bases/sport-history.base`'s `Previous sessions` view: up to five earlier `type: sport` journals from the same active org, newest first, across all sports. Drafts, archived/obsolete entries, and the current note are excluded. The date comparison uses the embedding note's `created`, so activation updates the history cutoff automatically. Existing sport notes appear in this history without migration. The Base's standalone `All sports` view lists personal sessions; older notes can opt into navigation by adding `![[sport-history.base#Previous sessions]]`.
...
QuickAdd navigation groups rarely used Person, Meeting, Project, and Team creation under `Entities`. Daily and Tasks live under `System` alongside startup refreshes. Native daily-note tooling is the normal entry point for daily notes; QuickAdd Daily is a secondary utility. Other capture choices stay at the top level; nested choices retain their IDs and command registrations.
